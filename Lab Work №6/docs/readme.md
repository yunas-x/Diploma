...
#### Factory
В контексте задачи необходимо парсить разные учебные планы, для создания экземпляров парсеров используется паттерн Factory для более удобного создания классов, реализующих ParserProtocol. Парсеры резолвятся с помощью встроенного в ParserFactory словаря с возможными парсерами.
...
```
#### Prototype
Используется для обновления данных авторизации на основе части предыдущих данных. Представим метод для создания объекта AuthData. В Python методы __copy__ и __deepcopy__ зашиты на уровне языка и переопределены с помощью рефлексии, поэтому в данном примере их наследование опущено.
...
#### Singleton
В данном случае этот паттерн может быть использован, чтобы создать обертку для объекта, отвечающего за доступ к базе данных. Используется метод __new__, чтобы переопределить инстанцирование объекта и хранить существующий объект, если он существует.
...
Более ортодоксальный вариант применения для кэширования данных о программе при запросах к Базе Данных
...
#### Facade
Данный паттерн используется работе для упрощения запросов к базе данных. Так как было произведено деление на микросервисы, логика получения данных усложнилась, было необходимо добавить запросы по сети для получения части данных об учебных планах и их аггрегации.
...
#### Adapter
При распиливании приложения на сервисы было необходимо изменить логику сбора данных в другом сервисе, хранящих справочные данные о направлениях подготовки. Для этого был добавлен адаптер, собирающий данные из внешних источников и выполнявший уже написанный код, в который передаются данные из внешнего источника.
...
#### Proxy
В данном примере используем для кэширования запросов к Базе Данных при авторизации для ускорения взаимодействия. Так как Python динамический язык, то по сути паттерн Proxy взаимозаменяем с паттерном Decorator
...
#### Template Method
Используем, чтобы декомпозировать верхнеуровнево парсинг учебных планов. Создадим три метода, один из которых читает служебную информацию (название программы, дату подписания и проч.), второй — основное содержание, в третий оборачивает полученные в первом и втором методе в общую структуру
...
```
#### Strategy
Используется для связывания парсинга и валидации данных по имеющейся модели. Класс CurriculaProcesser принимает экземпляр валидатора и экземпляр парсера
...
#### Command
Было необходимо более удобно обернуть запросы к внешним API, в том числе для их отложенного выполнения. Были созданы интерфейс и реализующий его класс, который инкапсулирует данные запроса и выполняет его при вызове метода send(). Также такой запрос можно отправлять повторно, что уменьшает дублирование.
...
• <b>Проблема: </b>необходимо заложить неизменяемость части интерфейсов при разделении системы на микросервисы, защитить реализацию паттерна Prototype от постоянного рефакторинга при смене аттрибутов и наследовании <br/>
• <b>Решение: </b>идентифицировать точки возможных изменений или неустойчивости и распределить обязанности так, чтобы обеспечить устойчивую работу системы. В данном случае были созданы дополнительные классы и использована рефлексия <br/>
...
### Polymorphism
• <b>Проблема: </b>необходимо избежать дублирования кода при проектировании метода добавления данных в базу<br/>
• <b>Решение: </b>распределить обязанности между классами с использованием полиморфных операций, оставив каждой внешней системе свой интерфейc. В данном случае выделен класс Query с методом add(), который позволяет добавлять любые данные, основываясь на переданном параметре <br/>
...
### Pure Fabrication
• <b>Проблема: </b> необходимо отделить логику работы с базой данных от классов, отвечающих за структуру данных и поведение системы <br/>
• <b>Решение: </b> синтезировать искусственную сущность для обеспечения Low Coupling и High Cohesion. В данном случае это все классы, задействующие ORM <br/> 
...
### Indirection
• <b>Проблема: </b> При разрастаниии системы появился хардкод парсеров и логики запросов к данных <br/>
• <b>Решение: </b>присвоить обязанности по обеспечению связи между компонентами или службами промежуточному объекту (см. Pure Fabrication) <br/> 
...
### Controller
• <b>Проблема: </b>необходимо обрабатывать вызовы API. Для этого использовался класс фремворка FastAPI, отвечающий за принятие запросов и валидацию данных <br/>
...
### Information Expert
• <b>Проблема: </b>Необходимо персистировать данные и собирать статистику <br/>
• <b>Решение: </b>Соответсвующие обязанности были переданы на классы, отвечающие за хранение данных (ORM и классы-модели данных для БД) <br/>
...
### Creator
• <b>Проблема: </b>Необходимо порождать экземпляр сессии для запроса к базе данных <br/>
• <b>Решение:  </b>Передавать Factory-объект для создания сессий в класс, отвечающий за запросы<br/>
...
### Low Coupling
• <b>Проблема: </b>необходимо разделить логику сбора, аггрегации данных, запросов к API и запросов между сервисами <br/>
• <b>Решение: </b>выделение отдельных пакетов, каждый из которых имеет свою ответственность <br/>
...
### High Cohesion
• <b>Проблема: </b>Необходимо сгруппировать похожую логику (запросы к БД и проч.) в соответствующие классы <br/>
• <b>Решение: </b>распределить обязанность классов, чтобы обеспечить высокое зацепление. Так, Query отвечает только за запросы к БД, тогда как класс Requester только за запросы по HTTP <br/>
